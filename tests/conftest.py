...
def disable_request_retry_delay():
    ...
    retryable = zigpy.util.retryable_request(tries=3, delay=0)
    with (
        patch("zha.zigbee.endpoint.RETRYABLE_REQUEST_DECORATOR", retryable),
        patch("zha.zigbee.cluster_io.RETRYABLE_REQUEST_DECORATOR", retryable),
    ):
        yield
